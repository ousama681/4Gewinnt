...
namespace VierGewinnt.Services
{
    ...
    public class MiniMaxAlgorithm
    {
        ...
        private List<int> ValidMoves(ref AIBoard board)
        {
        ...
        private bool IsTerminal(AIBoard board)
        {
            return board.WinningMove(board) != 0 || board.IsFull();
        }

        //Adjusts the score based on points scored by both players
        private void ConditionalScoreAdjustment(ref int player1, ref int player2, ref int score)
        {
        ...
        private int Evaluate(AIBoard board)
        {
            int score = 0;
            ...
            for (int i = 0; i < 6; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    ...
                    ConditionalScoreAdjustment(ref player1, ref player2, ref score);
                }
            }
        ...
        public ReturnMove GetQuickMove(AIBoard board)
        {
        ...
        private int Minimax(AIBoard board, int depth, int alpha, int beta, bool maximizingPlayer)
        {
            iterations++;
        ...
        public ReturnMove GetBestMove(AIBoard board)
        {
            iterations = 0;
            int bestScore = int.MinValue;
            int bestMove = 0;

            if (DebugMode) Debug.WriteLine("Evaluating board state...", "DEBUG", ConsoleColor.Green);
            ...
            if (DebugMode) Debug.WriteLine($"Best move: {bestMove} with score: {bestScore} Out of {iterations.ToString("N0")} iterations.", "DEBUG", ConsoleColor.Green);
